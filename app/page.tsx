...
export default function YampiConverter() {
  ...
  const gerarXlsBling = () => {
    ...
    const dadosBling = jsonObject.map(item => {
      ...
      const totalPedido = valorTotalLinha - desconto; 
  ...
  return (
    <>
      ...
      <div className={`min-h-screen flex flex-col items-center justify-start p-6 font-sans no-print transition-colors duration-300 ${bgMain} ${textMain}`}>
        ...
        {/* FOOTER MINIMALISTA */}
        <div className={`mt-8 pt-4 text-center text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
          <div className="flex flex-wrap items-center justify-center gap-2">
            <span>Versão {APP_VERSION}</span>
            <span className="opacity-30">—</span>
            <span>Desenvolvido por: {DEVELOPER_INFO.name}</span>
            <span className="opacity-30">—</span>
            <a 
